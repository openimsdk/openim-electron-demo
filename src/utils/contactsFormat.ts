...
const workerFunction = () => {
  ...
  const formatContacts = (data: FriendUserItem[], key = "nickname") => {
    ...
    return {
      indexList,
      dataList,
      totalList: dataList.flat(),
      groupCounts: dataList.map((data) => data.length),
    };
  };
...
type ContactsData = {
  indexList: string[];
  totalList: FriendUserItem[];
  groupCounts: number[];
};

export const formatContactsByWorker = (data: FriendUserItem[]): Promise<ContactsData> =>
  new Promise((resolve) => {
